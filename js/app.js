...
/* ---------- AI mockup generator (AI-only) ---------- */
let LAST_MOCKUP = null;   // {html, spec} of the most recent generation

// Only meaningful with AI on and a document analyzed. The button is hidden
// otherwise — the AI agents must analyze the BRD before a mockup can be built.
function canMockup(){ return !!(STATE && typeof AI!=='undefined' && AI.available && typeof Mockup!=='undefined'); }
function syncMockupButton(){ const b=E('mockup-btn'); if(b) b.classList.toggle('hidden', !canMockup()); }

async function generateMockup(){
  if(typeof Mockup==='undefined'){ toast('Mockup generator unavailable.'); return; }
  if(!STATE){ toast('Analyze a document first.'); return; }
  if(!AI.available){ toast('Turn on AI in Settings (⚙︎) to generate mockups.'); return; }
  const btn=E('mockup-btn'); const prev = btn?btn.textContent:'';
  if(btn){ btn.disabled=true; btn.textContent='✨ Analyzing BRD…'; }
  if(AI.onProgress===null) AI.onProgress=(m)=>{ if(btn) btn.textContent='✨ '+String(m).slice(0,28)+'…'; };
  try{
    const r = await Mockup.generate(STATE, { llm: llmJSON });
    LAST_MOCKUP = r;
    showMockup(r);
    toast('Mockup generated by '+AI.label+'.');
  }catch(e){
    toast('Could not generate mockup: '+(e.message||e));
  }finally{
    AI.onProgress=null;
    if(btn){ btn.disabled=false; btn.textContent=prev||'✨ Generate Mockup'; }
  }
}
...
function showMockup(r){
  ...
  frame.style.cssText='width:100%;height:72vh;border:0;border-radius:0 0 12px 12px;background:#f4f6f9';
  frame.srcdoc=r.html;
  body.innerHTML=''; body.appendChild(frame);
  modal.classList.remove('hidden');
}
function downloadMockup(){
  if(!LAST_MOCKUP) return;
  const name=((LAST_MOCKUP.spec&&LAST_MOCKUP.spec.app&&LAST_MOCKUP.spec.app.name)||'mockup').replace(/[^\w]+/g,'_').toLowerCase();
  const blob=new Blob([LAST_MOCKUP.html],{type:'text/html'});
  const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=name+'_mockup.html'; a.click(); URL.revokeObjectURL(a.href);
}
function openMockupTab(){
  if(!LAST_MOCKUP) return;
  const w=window.open('','_blank'); if(!w){ toast('Pop-up blocked — allow pop-ups to open the mockup.'); return; }
  w.document.open(); w.document.write(LAST_MOCKUP.html); w.document.close();
}
...
if(E('mockup-btn')) E('mockup-btn').onclick=generateMockup;
if(E('mockup-close')) E('mockup-close').onclick=()=>E('mockup-modal').classList.add('hidden');
if(E('mockup-download')) E('mockup-download').onclick=downloadMockup;
if(E('mockup-open')) E('mockup-open').onclick=openMockupTab;
if(E('mockup-modal')) E('mockup-modal').onclick=e=>{ if(e.target===E('mockup-modal')) E('mockup-modal').classList.add('hidden'); };
