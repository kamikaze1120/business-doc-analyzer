...
function osDocuments(p, body){
  ...
  const aiOn = (typeof AI!=='undefined' && AI.available && typeof Mockup!=='undefined');
  body.innerHTML=`
    ...
    <div class="card"><h3 class="sec" style="margin-top:0">✨ Interactive mockup</h3>
      ${aiOn
        ? `<p class="dim" style="margin:0 0 12px;font-size:12.5px">Let the AI agents read this project and generate a clickable UI prototype (roles, dashboard, submission form, approval workflow).</p>
           <button class="btn sm" id="os-mockup">✨ Generate Mockup (AI)</button>`
        : `<p class="dim" style="margin:0;font-size:12.5px">Turn on an AI provider in Settings (⚙︎) to generate an interactive mockup from this project.</p>`}
    </div>
  ...
  const mk=E('os-mockup'); if(mk) mk.onclick=async()=>{
    if(typeof AI==='undefined' || !AI.available){ toast('Turn on AI in Settings (⚙︎) first.'); return; }
    const ctx=Mockup.contextFromProject(p.id);
    if(!ctx || (!ctx.requirements.length && !ctx.objectives.length)){ toast('This project has no requirements or objectives yet to base a mockup on.'); return; }
    mk.disabled=true; const prev=mk.textContent; mk.textContent='✨ Analyzing project…';
    try{ const r=await Mockup.generate(null, {llm:llmJSON, context:ctx});
      if(typeof showMockup==='function') showMockup(r); else toast('Mockup ready.');
      toast('Mockup generated by '+AI.label+'.');
    }catch(e){ toast('Could not generate mockup: '+(e.message||e)); }
    finally{ mk.disabled=false; mk.textContent=prev; }
  };
